...
for x, y in product(range(300, 0, -1), repeat=2):
    grid_sum[(x, y)] = (
        grid[(x, y)]
        + grid_sum.get((x + 1, y), 0)
        + grid_sum.get((x, y + 1), 0)
        - grid_sum.get((x + 1, y + 1), 0)
    )
...
for size in range(1, 301):
    for x, y in product(range(1, 302 - size), repeat=2):
        power = (
            grid_sum[(x, y)]
            + grid_sum.get((x + size, y + size), 0)
            - grid_sum.get((x + size, y), 0)
            - grid_sum.get((x, y + size), 0)
        )
        if power > power_max:
            if (
                size == 3
            ):  # To save Part 1 answer, assumes 3x3 is better than 1x1 and 2x2
                max_point_3x3 = (x, y)
            power_max = power
            max_point = (x, y, size)

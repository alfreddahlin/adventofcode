...
acres = {
    (int(x), int(y)): input_data[y][x]
    for x, y in product(range(max_x), range(max_y))
    if input_data[y][x] in "|#"
}
...
def get_nearby(pos):
    nearby = [
        (a in woods, a in lumber)
        for a in product(range(pos[0] - 1, pos[0] + 2), range(pos[1] - 1, pos[1] + 2))
        if a != pos
    ]
...
while (str(woods), str(lumber)) not in states:
    ...
    ):  # Update state of acres (woods and lumber)

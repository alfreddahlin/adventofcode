...
while True:
    ...
    for m1 in moons:
        for m2 in range(m1+1,len(moons)):
            gravity = tuple(get_grav(moons[m1]['pos'][i],moons[m2]['pos'][i]) for i in range(3))
            moons[m1]['vel'] = tuple(v + dv for v, dv in zip(moons[m1]['vel'], gravity))
            moons[m2]['vel'] = tuple(v - dv for v, dv in zip(moons[m2]['vel'], gravity))
        moons[m1]['pos'] = tuple(p + v for p, v in zip(moons[m1]['pos'], moons[m1]['vel']))

    step += 1

    if step == 100:
        energy = [(abs(moons[m]['pos'][0]) +
                   abs(moons[m]['pos'][1]) +
                   abs(moons[m]['pos'][2])) *
                  (abs(moons[m]['vel'][0]) +
                   abs(moons[m]['vel'][1]) +
                   abs(moons[m]['vel'][2])) for m in moons]

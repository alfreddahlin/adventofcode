...
initial_cave = {
    (x, y): "."
    for x, y in product(range(len(input_data[0])), range(len(input_data)))
    if input_data[y][x] == "."
}
initial_units = {
    (x, y): {"race": input_data[y][x], "health": 200, "damage": 3, "tick": 0}
    for x, y in product(range(len(input_data[0])), range(len(input_data)))
    if input_data[y][x] in "EG"
}
...
def move(pos):
    stats = units.pop(pos)
    targets = [
        get_nearby(unit) for unit in units if units[unit]["race"] != stats["race"]
    ]
...
def attack(pos, stats):
    nearby = [
        point
        for point in get_nearby(pos)
        if point in units and stats["race"] != units[point]["race"]
    ]
    if nearby:
...
def get_path(pos, targets):
    if pos in targets:
        ...
        while queue:
            next_step = min(
                queue,
                key=lambda p: (len(path[p]), path[p][0][1], path[p][0][0])
                if path[p]
                else (0, p[1], p[0]),
            )
            new_steps = [
                step
                for step in get_nearby(next_step)
                if step in cave and (step not in seen or step in path)
            ]
            path[next_step].append(next_step)
            path.update(
                {step: path.get(step, list(path[next_step])) for step in new_steps}
            )
...
def get_nearby(pos):
    ...
    return [
        (x, y)
        for x, y in product(range(px - 1, px + 2), range(py - 1, py + 2))
        if abs(x - px + y - py) == 1
    ]
    ...
    for y in range(max(cave, key=lambda k: k[1])[1] + 2):
        ...
        for x in range(max(cave, key=lambda k: k[0])[0] + 2):
            if (x, y) in units:
                ...
                units_row += (
                    units[(x, y)]["race"] + "(" + str(units[(x, y)]["health"]) + "), "
                )
            elif (x, y) in cave:
                print(".", end="")
            else:
...
while damage[0] != damage[1]:
    ...
    while not all(
        [stats["race"] == list(units.values())[0]["race"] for stats in units.values()]
    ):
        unit = min(units, key=lambda k: (units[k]["tick"], k[1], k[0]))
        move(unit)
        if (
            sum([stats["race"] == "E" for stats in units.values()]) < num_elves
            and attempt != 3
        ):
            damage[0] = attempt + 1
            break

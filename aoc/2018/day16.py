...
instructions = [
    [list(map(int, codes[i : i + 4])) for i in range(0, len(codes), 4)]
...
for reg_in, instruction, reg_out in instructions:
    ...
    possibles.append(
        set(op for op in operations if op(values, list(reg_in)) == reg_out)
    )
...
while not all([len(v) == 1 for v in op_lookup.values()]):
    for code, unique in op_lookup.items():
        if len(unique) == 1:
            op_lookup.update(
                {
                    op: possible.difference(unique)
                    for op, possible in op_lookup.items()
                    if code != op
                }
            )

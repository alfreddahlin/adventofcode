...
data = [
    [
        re.findall(
            r"(\d+)[\s\S]+?(\d+)[\s\S]+?(?:\(([\s\S]+?)\)[\s\S]+?)?(\d+) ([a-z]+)[\s\S]+?(\d+)",
            group,
        )
        for group in army
    ]
    for army in input_data
]
data = [
    [
        re.findall(
            r"(\d+) units each with (\d+) hit points (?:\(([\s\S]+?)\))? ?with an attack that does (\d+) ([a-z]+) damage at initiative (\d+)",
            group,
        )
        for group in army
    ]
...
[
    groups_initial[int(group[-1])][mod.split(" ")[0]].add(word.strip(","))
    for group in data
    for mod in group[2].split("; ")
    for word in mod.split(" ")[2:]
]
...
while True:
    ...
    {
        groups[i].update({"damage": groups_initial[i]["damage"] + boost})
        for i in groups
        if groups[i]["army"] == "immune"
    }
    new_units = sum(group["units"] for group in groups.values())
    while not all(v["army"] == "immune" for v in groups.values()) and not all(
        v["army"] == "infect" for v in groups.values()
    ):
        ...
        while remaining:
            ...
            possible = {
                ...
                and groups[selecting]["army"] != props["army"]
                and get_damage(groups[selecting], props)
            }
            if possible:
                targets[selecting] = max(
                    possible,
                    key=lambda k: (
                        get_damage(groups[selecting], groups[k]),
                        get_power(groups[k]),
                        k,
                    ),
                )
